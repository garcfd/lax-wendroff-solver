      ...
      integer, parameter :: ni=400,nj=200,nk=1
      integer lcd(0:ni+1,0:nj+1)
      ...
      integer im1,jm1,ip1,jp1,ncut,ndead,mark
...
C-----mark cutcells
      ...
      do j = 1, nj
        ...
        if (lcd(i,j).eq.-1) then

          mark = 0

          do ii = i-1, i+1
          do jj = j-1, j+1
            if (lcd(ii,jj).eq.1) mark = 1
          enddo
          enddo
      ...
      do j = 1, nj
        ...
        if (lcd(i,j).eq.0) then
          ncut = ncut + 1
          cut(ncut,:) = [i,j] ! cutcell
        endif
...
C==============
!$acc data copy(r,u,v,p,t,lcd,cut,Q,E,F)
C==============
...
!$acc& default(none)
      ...
      if (lcd(i,j).eq.0) then  ! cutcell
        ...
        do jj = j-1,j+1
          ...
          if ( lcd(ii,jj).eq.1 ) then ! average from "live" cells
            ccln = ccln + 1
            rsum = rsum + r(ii,jj)
            psum = psum + p(ii,jj)
            tsum = tsum + t(ii,jj)
...
C-----step 4 smoothing
      if (.false.) then ! smoothing if needed
!$acc  parallel loop collapse(2)
!$acc& default(none)
      do i = 1, ni
      do j = 1, nj

        if (lcd(i,j).eq.1) then ! live
...
C-----lcd
      ...
      write(20)((lcd(i,j),i=1,ni),j=1,nj)
C-----dens
