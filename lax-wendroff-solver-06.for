      ...
      integer lcd(0:ni+1, 0:nj+1, 0:nk+1)

      real xn(ni+1), yn(nj+1), zn(nk+1)
      real u(ni,nj,nk),v(ni,nj,nk),w(ni,nj,nk)
      real r(ni,nj,nk),p(ni,nj,nk),t(ni,nj,nk)
      real Q(0:ni+1, 0:nj+1, 0:nk+1, 5)
      real E(0:ni+1, 0:nj+1, 0:nk+1, 5)
      real F(0:ni+1, 0:nj+1, 0:nk+1, 5)
      real G(0:ni+1, 0:nj+1, 0:nk+1, 5)
...
!$acc& default(none)
      ...
      do k = 1, nk
        ...
        if (lcd(i,j,k).eq.1) then ! live
          ...
          r(i,j,k) = urf*r(i,j,k) + (1-urf)*
     &   (r(im1,j)+r(ip1,j)+r(i,jm1)+r(i,jp1)+r(i,km1)+r(i,kp1))/6.0
          u(i,j,k) = urf*u(i,j,k) + (1-urf)*
     &   (u(im1,j)+u(ip1,j)+u(i,jm1)+u(i,jp1)+u(i,km1)+u(i,kp1))/6.0
          v(i,j,k) = urf*v(i,j,k) + (1-urf)*
     &   (v(im1,j)+v(ip1,j)+v(i,jm1)+v(i,jp1)+v(i,km1)+v(i,kp1))/6.0
          w(i,j,k) = urf*w(i,j,k) + (1-urf)*
     &   (w(im1,j)+w(ip1,j)+w(i,jm1)+w(i,jp1)+w(i,km1)+w(i,kp1))/6.0
          p(i,j,k) = urf*p(i,j,k) + (1-urf)*
     &   (p(im1,j)+p(ip1,j)+p(i,jm1)+p(i,jp1)+p(i,km1)+p(i,kp1))/6.0
          t(i,j,k) = urf*t(i,j,k) + (1-urf)*
     &   (t(im1,j)+t(ip1,j)+t(i,jm1)+t(i,jp1)+t(i,km1)+t(i,kp1))/6.0
        endif
...
C-----lcd
      ...
      write(20)((lcd(i,j,k),i=1,ni),j=1,nj)
C-----dens
      write(20)'SCALARS dens float  '//lf
      write(20)'LOOKUP_TABLE default'//lf
      write(20)((r(i,j,k),i=1,ni),j=1,nj)
C-----temp
      write(20)'SCALARS temp float  '//lf
      write(20)'LOOKUP_TABLE default'//lf
      write(20)((t(i,j,k),i=1,ni),j=1,nj)
C-----pres
      write(20)'SCALARS pres float  '//lf
      write(20)'LOOKUP_TABLE default'//lf
      write(20)((p(i,j,k),i=1,ni),j=1,nj)
C-----velocity
      write(20)'VECTORS vel float'//lf
      write(20)((u(i,j,k),v(i,j,k),0.0,i=1,ni),j=1,nj)
      close(20)
